...
int get_num_ints(char** argv)
{
	...
	while (fgets(num, 100, file1) != NULL)
	{
	         count1++; //increments count every time a number is counted
    	}
	...
	if (file2 == NULL)
		exit(EXIT_FAILURE); //makes sure file exists
	...
	if (count1 != count2) //makes sure the number of ints is the same if not returns -1
	{
		count1 = -1;
	}
                  ...
                  unsigned long int** output, int num_ints)
{
	...
	*output = (unsigned long int*) malloc(num_ints * sizeof(unsigned long int));
	if (input_one == NULL || input_two == NULL || output == NULL) //creates 3 arrays of dynamic size
		exit(EXIT_FAILURE);
}
...
{ 
  ...
  if (fp1 != NULL && fp2 != NULL)
  {
	...
	{
		fscanf(fp1, "%u", &input_one[i]); //grabs the numbers from both files and writes to both arrays
...
{
	for (int i = 0; i < num_ints; i++)
		output[i] = (unsigned long)input_one[i] +(unsigned long)input_two[i]; //take the sum of every element in the two arrays and write it to the output array
}
