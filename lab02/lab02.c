...
(6) Add the directory and file (with the exact same name, lab02/lab02.c) to
  your repo and push the solution by Saturday 11:59PM of the day of the lab.
**************************************************************/
...
{ 
  ...
  // testing capitalize_first_col from part(3)
  printf("\nCapitalize_first_col matrix:\n");
  apply_by_row(capitalize_first_col, data, data_rows, data_cols);

  // testing apply_by_col function from part(5)
  printf("\nBy column matrix:\n");
...
void capitalize_first_col(char* in_str, int row, int col, int rows, int cols)
{
  // in_str is just a pointer to an array of chars
  if (col == 0){
	int len = strlen(in_str);
  	// capitalize here 
  	for (int i = 0; i < len; i++){
  	printf("%c", toupper(in_str[i]));
  	}	
  	printf(" ");
  }
  else {
  ...
  // to uppercase ACSII flip the bit in the 32's place, 00->0<-0 0000, in hex that would be 0x0010
...
void apply_by_col(void (*func)(char*, int, int, int, int), char*** data, int rows, int cols)
{
  ...
  for (i = 0; i < cols; i++)
  {
	  for (j = 0; j < rows; j++)
	  {
		  // want to access elements column major rather than row so do data[j][i] instead of data[i][j]
