...

{
	...
	unsigned int *csr_col_ptr = (unsigned int*) malloc(sizeof(unsigned int) * nnz);
	double *csr_vals_ptr = (double*) malloc(sizeof(double) * nnz);
	for (int k = 0; k < m+1; k++) {
		row_ptr[k] = 0;
	}
	...
	unsigned int *cpy_row_ptr = (unsigned int*) malloc(sizeof(unsigned int) * (m+1));
	for (int i = 0; i <= m; i++) {
		cpy_row_ptr[i] = row_ptr[i];
	}

	// To get pointer offset we will use histogram
	  for (int i = 0; i < nnz; i++) {
		csr_col_ptr[cpy_row_ptr[row_ind[i]-1]] = col_ind[i]-1; // fill in array of column indices
		csr_vals_ptr[cpy_row_ptr[row_ind[i]-1]] = val[i]; // populate array of csr values
		cpy_row_ptr[row_ind[i]-1]++;
	  }

	  *csr_row_ptr = row_ptr;
	  *csr_col_ind = csr_col_ptr;
	  *csr_vals = csr_vals_ptr;
          ...
          double* vector_x, double* res)
{
    for (int i = 0; i < m; ++i) {
    	res[i]= 0.0;
	unsigned int nz_start = csr_row_ptr[i];
	unsigned int nz_end = csr_row_ptr[i+1];
	for (unsigned int j = nz_start; j < nz_end; j++) {
		res[i] += vector_x[csr_col_ind[j]] * csr_vals[j];
        	}
	}
}
