         ...
         int* color, int* distance, int* parent)
{
    ...
    adj_node_t ** q = malloc(sizeof(adj_node_t *));
    add_node(q, 0,create_node(source));

    while(1)
    {
	int vid = remove_node(q);

	if (vid == -1)
		break;
	if(list[vid] != NULL)
	{
		adj_node_t* adj_n = list[vid];

		while(adj_n !=NULL)
		{
			int adj_vid = adj_n->vid;
			if (color[adj_vid] == 0)
			{
				color[adj_vid] = 1;
				distance[adj_vid] = distance[vid] + 1;
				parent[adj_vid] = vid;
				add_node(q,0,create_node(adj_vid));
